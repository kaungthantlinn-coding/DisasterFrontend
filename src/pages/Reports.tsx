...
import { getAcceptedDisasterReports } from "../services/disasterReportService"; // <-- သင့် API service
...
export default function Reports() {
  ...
  useEffect(() => {
    async function fetchReports() {
      try {
        const data = await getAcceptedDisasterReports(); // <-- /api/DisasterReport/accepted
        setReports(data);
      } catch (error) {
  ...
  // High severity report တစ်ခုထုတ်ယူ featured လုပ်မယ်
  const featured = reports.find((r) => r.severity === SeverityLevel.High);
  // ကျန် reports
  const others = reports.filter((r) => r.id !== featured?.id);
  return (
    <div>
      <Header />
      <h2 className="text-2xl font-bold mb-8 text-gray-800 text-center">
        Latest news, stories and resources
      </h2>

      <div className="md:col-span-2 relative rounded-xl overflow-hidden shadow-lg grid md:grid-cols-2">
        {/* Featured High Card */}
        {featured && (
          <div className="md:col-span-2 relative rounded-xl overflow-hidden shadow-lg grid md:grid-cols-2">
            {featured.photoUrls && featured.photoUrls.length > 0 ? (
              <img
                src={featured.photoUrls[0]}
                alt={featured.title}
                className="w-full h-100 object-cover transform scale-110 translate-x-6"
              />
            ) : (
              <div className="w-full h-96 bg-red-100 flex items-center justify-center text-gray-500">
                No Image
              </div>
            )}
            <div className="bg-white  flex flex-col justify-center pl-20 ">
      ...
      {/* အနားဘက် စာအကြောင်း Cards */}
      <div className="mt-8 grid grid-cols-1 md:grid-cols-3 gap-6">
        {others.slice(0, 3).map((report) => (
          <div
            key={report.id}
      ...
      <div className="flex justify-center mt-10">
        <button className="px-6 py-3 bg-gray-100 rounded-lg text-sm font-medium text-gray-700 hover:bg-gray-200 transition">
          Load more items
        </button>
      </div>
    </div>
