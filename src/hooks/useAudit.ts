...
export const useCreateAuditLog = () => {
  ...
  return useMutation({
    mutationFn: (request: CreateAuditLogRequest) =>
...
export const useSearchAuditLogs = () => {
  const [searchResults, setSearchResults] = useState<AuditLogResponse | null>(
    null
  );
  const [isSearching, setIsSearching] = useState(false);

  const searchLogs = useCallback(
    async (query: string, filter?: AuditLogFilter) => {
      if (!query.trim()) {
...
export const useAuditLogger = () => {
  ...
  const logPermissionAction = useCallback(
    (
      ...
      return auditUtils.logPermissionAction(
        action,
        permission,
        target,
        details
  ...
  const logSecurityEvent = useCallback(
    (
      ...
      return auditUtils.logSecurityEvent(
        action,
        description,
        details,
        success,
...
export const useAuditLogFilter = () => {
  ...
  const setTargets = useCallback(
    (targetTypes: string[], targetIds?: string[]) => {
