...
export const useRoles = () => {
  ...
  const assignableRoles = useMemo(() => {
    ...
    const userWithPermissions =
      PermissionUtils.convertToUserWithPermissions(currentUser);
...
export const useUserPermissions = (userId?: string) => {
  ...
  const {
    ...
    queryFn: () =>
      userId ? permissionService.getUserWithPermissions(userId) : null,
    enabled: !!userId,
  ...
  const updatePermissionsMutation = useMutation({
    ...
    onSuccess: (updatedUser) => {
      queryClient.setQueryData(
        ["user-permissions", updatedUser.userId],
        updatedUser
      );
  ...
  const validatePermissionsMutation = useMutation({
    mutationFn: ({
      ...
      permissionService.validatePermissionChanges(
        userId,
        newPermissions,
        newRoleIds
      ),
...
export const useBulkPermissions = () => {
  ...
  const bulkUpdatePermissionsMutation = useMutation({
    ...
    onSuccess: (result) => {
      ...
      queryClient.invalidateQueries({ queryKey: ["permission-stats"] });
      toast.success(
        `Bulk permission update completed: ${result.successCount} successful, ${result.failureCount} failed`
      );
    },
    onError: (error: any) => {
      toast.error(
        error.response?.data?.message || "Bulk permission update failed"
      );
    },
  });
...
export const usePermissionAudit = (filters?: {
  ...
  const {
    ...
    queryFn: () =>
      permissionService.getPermissionAuditLog(page, pageSize, filters),
  ...
  const exportAuditMutation = useMutation({
    ...
    onError: (error: any) => {
      toast.error(
        error.response?.data?.message || "Failed to export audit log"
      );
    },
  });
...
export const usePermissionValidation = () => {
  const [validationResults, setValidationResults] = useState<
    Record<string, PermissionValidationResult>
  >({});

  const validatePermissionChange = useCallback(
    async (
      key: string,
      ...
      const result = PermissionUtils.validatePermissionChanges(
        user,
        newPermissions,
        currentUser
      );
