...
export const useDisasterNews = (
  options: UseDisasterNewsOptions = {}
): UseDisasterNewsReturn => {
  const {
    autoRefresh = false,
  ...
  const filterNews = useCallback(
    (newsItems: DisasterNewsItem[]): DisasterNewsItem[] => {
  ...
  const fetchNews = useCallback(async () => {
    ...
    try {
      ...
      if (mountedRef.current) {
        setError(
          err instanceof Error ? err.message : "Failed to fetch disaster news"
        );
