...
const RoleEditModal: React.FC<RoleEditModalProps> = ({ 
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const rolesChanged = JSON.stringify(roleNames.sort()) !== JSON.stringify(originalRoles.sort());

      console.log('RoleEditModal - Role Change Detection:', {
        roleNames,
        originalRoles,
        rolesChanged,
        roleNamesSorted: JSON.stringify(roleNames.sort()),
  ...
  const handleRemoveRole = (roleToRemove: string) => {
    ...
    const updatedRoles = roleNames.filter(
      role => role.toLowerCase() !== roleToRemove.toLowerCase()
    );

    console.log('RoleEditModal - Remove Role:', {
      removedRole: roleToRemove,
      previousRoles: roleNames,
      updatedRoles
    });
  ...
  const handleRoleChange = (role: string) => {
    ...
    console.log('RoleEditModal - Role Change:', {
      selectedRole: role,
      properCaseRole,
      previousRoles: roleNames,
      originalRoles,
    ...
    console.log('RoleEditModal - Updated Roles:', {
      existingRoles,
      ...
      <div className="bg-white rounded-2xl shadow-2xl max-w-lg w-full max-h-[90vh] overflow-y-auto">
        ...
        <form onSubmit={handleSubmit} className="p-6 space-y-6">
          ...
          <div className="flex items-center space-x-4">
            ...
            />
            <div>
              <h3 className="text-lg font-semibold text-gray-900">Editing Role: {user.name}</h3>
          ...
          <div>
            ...
            >
              ...
              {availableRoles
                .filter(role => !roleNames.some(r => r.toLowerCase() === role.toLowerCase()))
                .map(role => (
