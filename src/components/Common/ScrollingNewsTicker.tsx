...
const ScrollingNewsTicker: React.FC<ScrollingNewsTickerProps> = ({
  ...
  const createNewsText = () => {
    if (news.length === 0) return 'No recent disaster alerts - All systems monitoring normally';
  ...
  const createClickableNewsItems = () => {
    if (news.length === 0) {
      return (
        <span className="text-white/90">No recent disaster alerts - All systems monitoring normally</span>
      );
    }
  ...
  if (loading && news.length === 0) {
    return (
      <div className={`bg-gradient-to-r from-blue-600 to-indigo-600 text-white py-2 overflow-hidden ${className}`}>
        <div className="container mx-auto px-4">
          <div className="flex items-center justify-center">
            <RefreshCw className="animate-spin mr-2" size={16} />
            <span className="text-sm font-medium">Loading live disaster alerts...</span>
          </div>
        </div>
      </div>
    );
  ...
  if (error && news.length === 0) {
    return (
      <div className={`bg-gradient-to-r from-red-600 to-red-700 text-white py-2 overflow-hidden ${className}`}>
        <div className="container mx-auto px-4">
          <div className="flex items-center justify-center">
            <AlertTriangle className="mr-2" size={16} />
            <span className="text-sm font-medium">Unable to load disaster news</span>
            <button 
              onClick={refresh}
  ...
  return (
    <div className={`bg-gradient-to-r from-red-900 via-red-800 to-red-900 text-white py-2 overflow-hidden relative ${className}`}>
      {/* Breaking News Label */}
