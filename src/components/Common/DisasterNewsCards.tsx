...
const DisasterNewsCards: React.FC<DisasterNewsCardsProps> = ({
  ...
  if (loading && news.length === 0) {
    return (
      <div className={`bg-gradient-to-r from-slate-900 to-blue-900 text-white py-8 ${className}`}>
        <div className="container mx-auto px-4">
          <div className="flex items-center justify-center">
            <RefreshCw className="animate-spin mr-3" size={24} />
            <span className="text-lg font-medium">Loading disaster updates...</span>
          </div>
        </div>
      </div>
    );
  ...
  if (error && news.length === 0) {
    return (
      <div className={`bg-gradient-to-r from-red-900 to-red-800 text-white py-8 ${className}`}>
        <div className="container mx-auto px-4">
          <div className="text-center">
            <AlertTriangle className="mx-auto mb-4" size={32} />
            <p className="text-lg font-medium mb-4">Unable to load disaster updates</p>
            <button 
              onClick={refresh}
  ...
  if (news.length === 0) {
    return (
      <div className={`bg-gradient-to-r from-green-900 to-emerald-800 text-white py-8 ${className}`}>
        <div className="container mx-auto px-4">
          <div className="flex items-center justify-center">
            <Globe className="mr-3" size={24} />
            <span className="text-lg font-medium">No current disaster alerts - All systems monitoring normally</span>
          </div>
        </div>
      </div>
    );
  ...
  return (
    <div className={`bg-gradient-to-r from-slate-900 via-red-900 to-slate-900 text-white py-6 relative overflow-hidden ${className}`}>
      {/* Background Pattern */}
                  ...
                  {news
                    ...
                    .map((item, index) => (
                      <div
                        ...
                        className={`rounded-xl p-4 border-2 transition-all duration-300 hover:scale-105 cursor-pointer disaster-news-item ${getSeverityBackground(item.severity)} group`}
