...
export const authApi = {
  ...
  verifyResetToken: async (
    token: string
  ): Promise<{ valid: boolean; message?: string; errorType?: string }> => {
    try {
      ...
      if (token.length < 20) {
        return {
          valid: false,
          message:
            "Invalid token format. Please use the complete link from your email.",
      ...
      const result = {
        ...
        errorType: response.data.isValid
          ? undefined
      ...
      if (error.response?.status === 500) {
        return {
          valid: false,
          message:
            "Server error occurred. Please try again later or request a new reset link.",
