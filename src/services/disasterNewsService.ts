...
class DisasterNewsService {
  ...
  private cache: Map<string, { data: DisasterNewsItem[]; timestamp: number }> =
    new Map();
  ...
  async getEarthquakeData(): Promise<DisasterNewsItem[]> {
    ...
    try {
      ...
      const earthquakes: DisasterNewsItem[] =
        data.features
          ?.map((feature: any) => ({
            id: feature.id,
  ...
  async getNASAEvents(): Promise<DisasterNewsItem[]> {
    ...
    try {
      const response = await fetch(
        "https://eonet.gsfc.nasa.gov/api/v3/events?limit=30&status=open"
      );
      const data = await response.json();

      const events: DisasterNewsItem[] =
  ...
  async getReliefWebDisasters(): Promise<DisasterNewsItem[]> {
    ...
    try {
      const response = await fetch(
        "https://api.reliefweb.int/v1/disasters?appname=DisasterWatch&profile=list&preset=latest&limit=30"
      );
      const data = await response.json();

      const disasters: DisasterNewsItem[] =
        data.data?.map((item: any) => ({
          id: item.id,
          title: `${item.fields.name} - ${
            item.fields.country?.[0]?.name || "Unknown Location"
          }`,
          source: "ReliefWeb",
  ...
  async getAllDisasterNews(): Promise<DisasterNewsItem[]> {
    try {
      const [earthquakes, nasaEvents, reliefWebEvents] =
  ...
  private getCategorySeverity(
    ...
    if (category.includes("severe") || category.includes("major"))
      return "critical";
    if (category.includes("wildfire") || category.includes("volcano"))
      return "high";
    return "medium";
  }

  private getReliefWebSeverity(
    status: string
  ): "low" | "medium" | "high" | "critical" {
    if (!status) return "medium";
    const statusLower = status.toLowerCase();
    if (statusLower.includes("severe") || statusLower.includes("emergency"))
      return "critical";
  ...
  private mapNASACategory(categories: any[]): DisasterNewsItem["type"] {
    ...
    if (category.includes("storm") || category.includes("severe"))
      return "storm";
  ...
  private mapReliefWebType(type: string): DisasterNewsItem["type"] {
    ...
    if (
      ...
      return "cyclone";
    if (typeLower.includes("wildfire") || typeLower.includes("fire"))
      return "wildfire";
  ...
  private getLocationFromCoordinates(coordinates: number[]): string {
    ...
    if (lat > 23.5 && lat < 71 && lng > -180 && lng < -30)
