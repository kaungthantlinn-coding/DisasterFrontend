...
export const permissionService = {
  ...
  async duplicateRole(roleId: string, newName: string): Promise<Role> {
    const response = await apiClient.post(`/api/admin/roles/${roleId}/duplicate`, {
      name: newName
    });
    return response.data;
  },
  ...
  async updateUserPermissions(data: UpdateUserPermissionsDto): Promise<UserWithPermissions> {
    const response = await apiClient.put(`/api/admin/users/${data.userId}/permissions`, data);
    return response.data;
  },

  async assignRolesToUser(data: AssignRoleDto): Promise<UserWithPermissions> {
    const response = await apiClient.post(`/api/admin/users/${data.userId}/roles`, data);
    return response.data;
  },

  async revokeRolesFromUser(userId: string, roleIds: string[], reason?: string): Promise<UserWithPermissions> {
    const response = await apiClient.delete(`/api/admin/users/${userId}/roles`, {
      data: { roleIds, reason }
    });
    return response.data;
  },
  ...
  // Bulk Operations
  async bulkAssignRoles(data: BulkRoleAssignmentDto): Promise<{ successCount: number; failureCount: number; errors: string[] }> {
    const response = await apiClient.post('/api/admin/users/bulk/roles', data);
    return response.data;
  },

  async bulkUpdatePermissions(operations: BulkPermissionOperation[]): Promise<{ successCount: number; failureCount: number; errors: string[] }> {
    const response = await apiClient.post('/api/admin/permissions/bulk', { operations });
    return response.data;
  },
  ...
  async validateRoleAssignment(
    ...
    const response = await apiClient.post('/api/admin/roles/validate-assignment', {
      userId,
      roleIds
    });
    return response.data;
  ...
  async getPermissionAuditLog(
    ...
    const response = await apiClient.get(`/api/admin/audit/permissions?${params}`);
    return response.data;
  },
  ...
  async exportAuditLog(
    ...
    const response = await apiClient.get(`/api/admin/audit/permissions/export?${params}`, {
      responseType: 'blob'
    });
    return response.data;
  },
  ...
  async getPermissionDependencies(permission: Permission): Promise<{
    ...
    const response = await apiClient.get(`/api/admin/permissions/${permission}/dependencies`);
    return response.data;
  },
  ...
  async searchUsersWithPermissions(
    ...
    const response = await apiClient.post('/api/admin/users/search/permissions', {
      query,
      filters
    });
    return response.data;
  ...
  async createRoleFromTemplate(templateId: string, customizations: {
    ...
    const response = await apiClient.post(`/api/admin/roles/templates/${templateId}/create`, customizations);
    return response.data;
  },
  ...
  async getPermissionInheritance(userId: string): Promise<{
    ...
    const response = await apiClient.get(`/api/admin/users/${userId}/permissions/inheritance`);
    return response.data;
  },
  ...
  async quickAssignRole(userId: string, roleName: string): Promise<void> {
    await apiClient.post(`/api/admin/users/${userId}/quick-assign-role`, { roleName });
  },

  async quickRevokeRole(userId: string, roleName: string): Promise<void> {
    await apiClient.post(`/api/admin/users/${userId}/quick-revoke-role`, { roleName });
  },

  async quickGrantPermission(userId: string, permission: Permission): Promise<void> {
    await apiClient.post(`/api/admin/users/${userId}/quick-grant-permission`, { permission });
  },

  async quickRevokePermission(userId: string, permission: Permission): Promise<void> {
    await apiClient.post(`/api/admin/users/${userId}/quick-revoke-permission`, { permission });
  }
};
