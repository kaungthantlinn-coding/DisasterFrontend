...
export interface AuditServiceInterface {
  // Audit log retrieval
  getAuditLogs(filter?: AuditLogFilter, page?: number, pageSize?: number): Promise<AuditLogResponse>;
  ...
  // Bulk operations
  createBulkAuditLogs(requests: CreateAuditLogRequest[]): Promise<AuditLogEntry[]>;
  
  // Export functionality
  exportAuditLogs(options: AuditLogExportOptions): Promise<{ downloadUrl: string; fileName: string }>;
  
  // Search and filtering
  searchAuditLogs(query: string, filter?: AuditLogFilter): Promise<AuditLogResponse>;
  getAuditLogsByUser(userId: string, filter?: AuditLogFilter): Promise<AuditLogResponse>;
  getAuditLogsByTarget(targetType: string, targetId: string, filter?: AuditLogFilter): Promise<AuditLogResponse>;
  
  // Alert rules
  getAlertRules(): Promise<AuditAlertRule[]>;
  createAlertRule(rule: Omit<AuditAlertRule, 'id' | 'createdAt' | 'updatedAt' | 'createdBy'>): Promise<AuditAlertRule>;
  updateAlertRule(id: string, rule: Partial<AuditAlertRule>): Promise<AuditAlertRule>;
  deleteAlertRule(id: string): Promise<void>;
  testAlertRule(id: string): Promise<{ success: boolean; message: string }>;
  
  // Compliance reporting
  generateComplianceReport(reportType: 'GDPR' | 'SOX' | 'HIPAA' | 'CUSTOM', period: { start: string; end: string }): Promise<AuditComplianceReport>;
  getComplianceReports(): Promise<AuditComplianceReport[]>;
  downloadComplianceReport(reportId: string): Promise<{ downloadUrl: string; fileName: string }>;
  
  // Analytics
  getActivityTrends(period: 'day' | 'week' | 'month', filter?: AuditLogFilter): Promise<{
    labels: string[];
    datasets: Array<{
      label: string;
      data: number[];
  ...
  getTopActors(limit?: number, filter?: AuditLogFilter): Promise<Array<{
    actorId: string;
    actorName: string;
    actorEmail: string;
    actionCount: number;
  ...
  archiveOldLogs(beforeDate: string): Promise<{ archivedCount: number; archiveLocation: string }>;
...
class AuditService implements AuditServiceInterface {
  ...
  async getAuditLogs(filter?: AuditLogFilter, page = 1, pageSize = 50): Promise<AuditLogResponse> {
  ...
  async createBulkAuditLogs(requests: CreateAuditLogRequest[]): Promise<AuditLogEntry[]> {
    const response = await apiClient.post(`${this.baseUrl}/logs/bulk`, { logs: requests });
    return response.data;
  }

  async exportAuditLogs(options: AuditLogExportOptions): Promise<{ downloadUrl: string; fileName: string }> {
    const response = await apiClient.post(`${this.baseUrl}/export`, options);
    return response.data;
  }

  async searchAuditLogs(query: string, filter?: AuditLogFilter): Promise<AuditLogResponse> {
  ...
  async getAuditLogsByUser(userId: string, filter?: AuditLogFilter): Promise<AuditLogResponse> {
    ...
    const response = await apiClient.get(`${this.baseUrl}/users/${userId}/logs?${params.toString()}`);
    return response.data;
  }

  async getAuditLogsByTarget(targetType: string, targetId: string, filter?: AuditLogFilter): Promise<AuditLogResponse> {
    ...
    const response = await apiClient.get(`${this.baseUrl}/targets/${targetType}/${targetId}/logs?${params.toString()}`);
    return response.data;
  }
  ...
  async createAlertRule(rule: Omit<AuditAlertRule, 'id' | 'createdAt' | 'updatedAt' | 'createdBy'>): Promise<AuditAlertRule> {
  ...
  async updateAlertRule(id: string, rule: Partial<AuditAlertRule>): Promise<AuditAlertRule> {
    const response = await apiClient.put(`${this.baseUrl}/alerts/rules/${id}`, rule);
    return response.data;
  }
  ...
  async testAlertRule(id: string): Promise<{ success: boolean; message: string }> {
    const response = await apiClient.post(`${this.baseUrl}/alerts/rules/${id}/test`);
    return response.data;
  }
  ...
  async downloadComplianceReport(reportId: string): Promise<{ downloadUrl: string; fileName: string }> {
    const response = await apiClient.get(`${this.baseUrl}/compliance/reports/${reportId}/download`);
    return response.data;
  }

  async getActivityTrends(period: 'day' | 'week' | 'month', filter?: AuditLogFilter) {
    ...
    const response = await apiClient.get(`${this.baseUrl}/analytics/trends?${params.toString()}`);
    return response.data;
  }
  ...
  async getTopActors(limit = 10, filter?: AuditLogFilter) {
    ...
    const response = await apiClient.get(`${this.baseUrl}/analytics/top-actors?${params.toString()}`);
    return response.data;
  }
  ...
  async getFailureAnalysis(filter?: AuditLogFilter) {
    ...
    const response = await apiClient.get(`${this.baseUrl}/analytics/failures?${params.toString()}`);
    return response.data;
  }
  ...
  async cleanupExpiredLogs(): Promise<{ deletedCount: number; message: string }> {
    const response = await apiClient.post(`${this.baseUrl}/maintenance/cleanup`);
    return response.data;
  }

  async archiveOldLogs(beforeDate: string): Promise<{ archivedCount: number; archiveLocation: string }> {
    const response = await apiClient.post(`${this.baseUrl}/maintenance/archive`, { beforeDate });
    return response.data;
  }
...
export const auditUtils = {
  ...
  logUserAction: async (
    ...
    return auditService.createAuditLog({
      ...
      description: `${action.replace('_', ' ').toLowerCase()} for user ${targetUser.name}`,
      details,
      oldValues,
      newValues,
      success: true
  ...
  logRoleAction: async (
    ...
    return auditService.createAuditLog({
      ...
      description: `${action.replace('_', ' ').toLowerCase()} for role ${targetRole.name}`,
      details,
      oldValues,
      newValues,
      success: true
  ...
  logPermissionAction: async (
    ...
    return auditService.createAuditLog({
      ...
      description: `${action.replace('_', ' ').toLowerCase()} permission ${permission} for ${target.type} ${target.name}`,
      details: {
        permission,
        ...details
      },
