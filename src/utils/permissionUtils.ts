...
export class PermissionUtils {
   ...
   */
  static hasPermission(user: UserWithPermissions | User, permission: Permission): boolean {
    ...
    if ('roles' in user && Array.isArray(user.roles)) {
      ...
      return roleNames.some(roleName => {
        const rolePermissions = DEFAULT_ROLE_PERMISSIONS[roleName.toLowerCase()];
   ...
   */
  static hasAnyPermission(user: UserWithPermissions | User, permissions: Permission[]): boolean {
   ...
   */
  static hasAllPermissions(user: UserWithPermissions | User, permissions: Permission[]): boolean {
  ...
  static getEffectivePermissions(user: UserWithPermissions): Permission[] {
    ...
    const allPermissions = [...new Set([...rolePermissions, ...user.directPermissions])];
  ...
  static validatePermissionChanges(
    ...
    if (!this.hasPermission(currentUser, Permission.MANAGE_PERMISSIONS)) {
      ...
      result.blockers.push('You do not have permission to manage user permissions');
      return result;
    }
    ...
    if (user.userId === currentUser.userId) {
      result.warnings.push('You are modifying your own permissions. This may affect your access.');
    }
  ...
  static convertToUserWithPermissions(user: User): UserWithPermissions {
    ...
    if (user.roles && Array.isArray(user.roles)) {
      user.roles.forEach(roleName => {
        if (typeof roleName === 'string') {
          const permissions = DEFAULT_ROLE_PERMISSIONS[roleName.toLowerCase()] || [];
          roles.push({
            ...
            permissions,
            isSystemRole: Object.values(SYSTEM_ROLES).includes(roleName.toLowerCase() as any),
            isActive: true,
    ...
    const effectivePermissions = this.calculateEffectivePermissions(roles, directPermissions);
   ...
   */
  static getAssignableRoles(roles: Role[], currentUser: UserWithPermissions): Role[] {
  ...
  static formatPermissionsForDisplay(permissions: Permission[]): string {
    ...
    if (permissions.length === 1) return this.getPermissionDisplayName(permissions[0]);
    if (permissions.length <= 3) {
      return permissions.map(p => this.getPermissionDisplayName(p)).join(', ');
    }
    return `${permissions.slice(0, 2).map(p => this.getPermissionDisplayName(p)).join(', ')} and ${permissions.length - 2} more`;
  }
 ...
 */
export const createPermissionChecker = (user: UserWithPermissions | User | null) => {
  return {
    ...
    isSuperAdmin: () => 
      user ? PermissionUtils.isSuperAdmin(user) : false,
    canAssignRole: (role: Role) => 
      user ? PermissionUtils.canAssignRole(role, user as UserWithPermissions) : false
  };
};