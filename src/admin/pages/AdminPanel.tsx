...
const QuickActionCard: React.FC<QuickActionProps> = ({ icon, title, description, link, color, badge }) => {
  return (
    <Link
      to={link}
    ...
    >
      <div className="flex items-start justify-between mb-4">
        <div className={`p-2.5 bg-gradient-to-br ${color} rounded-lg text-white group-hover:scale-105 transition-transform duration-200`}>
          {icon}
        </div>
        {badge && (
...
const AdminPanel: React.FC = () => {
  ...
  const navigationItems = [
    ...
    },
    {
      ...
      badge: stats?.pendingReports && stats.pendingReports > 20 ? stats.pendingReports.toString() : null
    },
    {
      name: 'Analytics',
        ...
        <div className="flex flex-col lg:flex-row lg:items-center lg:justify-between">
          <div className="mb-6 lg:mb-0">
            ...
            <p className="text-slate-600 leading-relaxed max-w-2xl">
              Your disaster response system is running smoothly. Here's your comprehensive overview of today's activities and system performance.
            </p>
          </div>
          
          <div className="flex flex-col sm:flex-row items-start sm:items-center space-y-3 sm:space-y-0 sm:space-x-3">
            <div className={`flex items-center px-3 py-2 rounded-lg text-sm font-medium border ${
              isOnline 
      ...
      <div className="space-y-6">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          ...
          <QuickActionCard
            ...
            badge={stats?.pendingReports && stats.pendingReports > 20 ? 'Urgent' : undefined}
          />
          <QuickActionCard
            ...
            {[
              ...
              <div key={index} className="flex items-center space-x-3 p-3 hover:bg-slate-50 rounded-lg transition-colors">
                <div className={`p-1.5 rounded-md ${
                  ...
                  activity.type === 'report' ? 'bg-emerald-50 text-emerald-600' :
                  ...
                  {activity.type === 'user' ? <Users className="w-3.5 h-3.5" /> :
                   activity.type === 'report' ? <FileText className="w-3.5 h-3.5" /> :
                   activity.type === 'system' ? <Settings className="w-3.5 h-3.5" /> :
                   activity.type === 'security' ? <Shield className="w-3.5 h-3.5" /> :
        ...
        <div className="bg-white rounded-xl border border-slate-100 p-6">
          <div className="flex items-center justify-between mb-6">
            ...
            <div className="flex items-center space-x-2">
              ...
              <span className="text-xs text-slate-500 uppercase tracking-wide font-medium">Operational</span>
            </div>
          </div>
          ...
          <div className="space-y-4">
            {[
              ...
              { name: 'Storage Usage', value: 45, unit: '%', status: 'excellent' },
            ].map((metric, index) => (
              <div key={index} className="space-y-2">
                ...
                <div className="w-full bg-slate-100 rounded-full h-1.5">
                  <div
                    ...
                    style={{
                      width: `${metric.unit === 'ms' ? Math.min((1000 - metric.value) / 10, 100) : metric.unit === '%' ? 100 - metric.value : metric.value}%`
                    }}
                  />
                </div>
              </div>
      ...
      <div className="min-h-screen bg-gradient-to-br from-slate-50 to-slate-100">
        ...
        <div className={`hidden lg:fixed lg:inset-y-0 lg:flex lg:flex-col transition-all duration-300 z-40 ${
          ...
          <div className="flex flex-col flex-1 bg-white border-r border-slate-100">
            {/* Sidebar header */}
            <div className={`flex items-center ${sidebarCollapsed ? 'justify-center' : 'justify-between'} p-4 border-b border-slate-100`}>
              {!sidebarCollapsed && (
                <div className="flex items-center space-x-2">
                ...
                {navigationItems.map((item) => {
                  ...
                  return (
                    <Link
                      key={item.name}
                      to={item.href}
                      className={`flex items-center ${sidebarCollapsed ? 'justify-center px-3' : 'justify-between px-3'} py-2.5 rounded-lg text-sm font-medium transition-all duration-200 group ${
                        item.current
                    ...
                    >
                      <div className="flex items-center">
                        <Icon className={`w-4 h-4 ${sidebarCollapsed ? '' : 'mr-3'} transition-colors`} />
        ...
        <div className={`transition-all duration-300 ${
          ...
          <div className="sticky top-0 z-30 bg-white/80 backdrop-blur-lg shadow-sm border-b border-slate-200">
            <div className="flex items-center justify-between px-6 py-4">
              ...
              <div className="flex items-center space-x-3">
                <div className={`flex items-center px-3 py-2 rounded-xl text-sm font-medium transition-colors ${
                  isOnline 
