...
const Dashboard: React.FC = () => {
  ...
  const stats = {
    reportsSubmitted: userReportsData?.length || 0,
    verifiedReports:
      userReportsData?.filter((r: any) => r.status === "verified").length || 0,
    assistanceProvided: 0, // This would come from assistance tracking API
  };

  // Get user's reports from API data
  const myReports = (userReportsData || [])
    .slice(0, 2)
    .map((report: any) => ({
      id: report.id,
      title: report.title,
      ...
      status:
        report.status === "resolved"
          ...
          : ((report.status.charAt(0).toUpperCase() +
              report.status.slice(1)) as "Verified" | "Pending" | "Rejected"),
